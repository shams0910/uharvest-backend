...
class CropChoicesWithCropGroups(APIView):
    def get(self, request):
        ...
        crop_groups = list({'id': value, 'name': text}
                           for value, text in crop_groups)
...
class CropChoicesWithTasks(APIView):
    def get(self, request, region):
        ...
        last_7_days = tuple(
            (today - datetime.timedelta(days=n)).strftime('%Y-%m-%d') for n in range(0, 7))
        ...
        for task in tasks:
            tp_of_current_task = [
                tp for tp in tp_of_last_7_days_serialized if tp['id'] == task['id']]
            existing_dates = [tp['date'].strftime(
                '%Y-%m-%d') for tp in tp_of_current_task]

            for date in last_7_days:
                if date in existing_dates:
                    pass
                else:
                    tp_of_current_task.append(
                        {'id': None, "date": date, 'sum': 0})
        ...
        for cropchoice in cropchoices:
            tasks_of_current_cropchoice = [
                t for t in tasks if t['crop_choice_id'] == cropchoice['id']]
...
class CropsInContour(APIView):
    def get(self, request, pk):
        crops_in_contour = Crop.objects.select_related(
            'crop_choice').filter(contour_id=pk)
        serialized_crops_in_contour = CropSerializer(
            crops_in_contour, many=True).data
...
class CreateTaskProgressesInDate(APIView):
    ...
    def post(self, request):
        ...
        if year and type(year) == int:
            crop_ids = Crop.objects.filter(
                year=year, contour__supervisor_id=request.user.id, crop_choice_id=cropchoice_id).values_list('id', flat=True)
            tps = []
...
class TaskProgressesInContour(APIView):
    def get(self, request, pk):
        task_progresses_in_contour = TaskProgress.objects.filter(
            crop__contour__id=pk)
...
class TaskProgressesOfTaskByContoursInYear(APIView):
    def get(self, request, task_id, year):
        task_progresses = TaskProgress.objects.select_related('crop__contour__farmer__user').filter(
            task_id=task_id, crop__year=year, crop__contour__supervisor_id=request.user.id)
        serialized_task_progresses = TaskProgressWithContourSerializer(
            task_progresses, many=True).data
